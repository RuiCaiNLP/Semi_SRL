...
from nnet.nn_models.base_learner.SRL_CVT_biaffinePI import BiLSTMTagger
...
class SRLRunner(Runner):
    ...
    def get_converter(self):
        def bio_converter(batch):
            ...
            sent_pred_indicator = np.zeros(sent_batch.shape, dtype='int64')-1
            for r, row in enumerate(sent_pred_lemmas_idx):
                for c, column in enumerate(row):
                    sent_pred_indicator[r][c] = 0
