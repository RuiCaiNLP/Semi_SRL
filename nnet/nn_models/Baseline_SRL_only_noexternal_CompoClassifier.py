...
class BiLSTMTagger(nn.Module):
    ...
    def __init__(self, hps, *_):
        ...
        self.role_dim = role_embedding_dim
        ...
        self.W_R = nn.Parameter(torch.rand(self.tagset_size, self.role_dim))

        # non-linear map to role embedding
        self.role_map = nn.Linear(in_features=role_embedding_dim * 2, out_features=400)
    ...
    def forward(self, sentence, p_sentence,  pos_tags, lengths, target_idx_in, region_marks,
        ...
        frame_ids = frames[:, 0]
        frame_embeds = self.frame_embeddings(frame_ids)
        # T * B * H
        added_embeds = torch.zeros(self.tagset_size, hidden_states_3.size()[0], self.region_embeddings).to(
            device)
        frame_embeds = added_embeds + frame_embeds

        added_embeds = torch.zeros(hidden_states_3.size()[0], self.tagset_size, self.region_embeddings).to(
            device)
        role_embeds = added_embeds + self.W_R
