...
class BiLSTMTagger(nn.Module):
    ...
    def __init__(self, hps, *_):
        ...
        self.dep_embeddings = nn.Embedding(self.dep_size, 16)
        self.link_embeddings = nn.Embedding(4, 16)
        ...
        self.num_layers = 3
        self.BiLSTM_SRL = nn.LSTM(input_size=sent_embedding_dim_SRL + 16*2, hidden_size=lstm_hidden_dim, batch_first=True,
                                    bidirectional=True, num_layers=self.num_layers)
    ...
    def forward(self, sentence, p_sentence,  pos_tags, lengths, target_idx_in, region_marks,
        ...
        dep_embeds = self.dep_embeddings(Predicate_Labels_nd)
        link_embeds = self.link_embeddings(Predicate_link)
        SRL_hidden_states = torch.cat((embeds_SRL,  fixed_embeds, region_marks, dep_embeds, link_embeds), 2)
