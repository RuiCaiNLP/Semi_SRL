...
class BiLSTMTagger(nn.Module):
    ...
    def __init__(self, hps, *_):
        ...
        self.Non_Predicate_Proj = nn.Linear(2 * lstm_hidden_dim, 300)
        self.Predicate_Proj = nn.Linear(2 * lstm_hidden_dim, 300)
        ...
        self.label_dropout_3 = nn.Dropout(p=0.2)
        self.label_dropout_4 = nn.Dropout(p=0.2)
        ...
        self.W_R = nn.Parameter(torch.rand(300 + 1, self.tagset_size * (300 + 1)))
    ...
    def forward(self, sentence, p_sentence,  pos_tags, lengths, target_idx_in, region_marks,
        ...
        hidden_states_word = self.dropout_1(F.leaky_relu(self.Non_Predicate_Proj(hidden_states_3), 0.1))
        predicate_embeds = hidden_states_3[np.arange(0, hidden_states_3.size()[0]), target_idx_in]
        hidden_states_predicate = self.dropout_2(F.leaky_relu(self.Predicate_Proj(predicate_embeds), 0.1))
