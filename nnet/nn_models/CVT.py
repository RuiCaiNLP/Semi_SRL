...
class BiLSTMTagger(nn.Module):
    ...
    def __init__(self, hps, *_):
        ...
        self.W_R = nn.Bilinear(lstm_hidden_dim, lstm_hidden_dim, self.tagset_size)
        ...
        self.W_R_FF = nn.Bilinear(self.cvt_hidden_dim,  self.cvt_hidden_dim, self.tagset_size)
        ...
        self.W_R_BB = nn.Bilinear(self.cvt_hidden_dim, self.cvt_hidden_dim, self.tagset_size)
        ...
        self.W_R_FB = nn.Bilinear(self.cvt_hidden_dim, self.cvt_hidden_dim, self.tagset_size)
        ...
        self.W_R_BF = nn.Bilinear(self.cvt_hidden_dim, self.cvt_hidden_dim, self.tagset_size)
    ...
    def Semi_SRL_Loss(self, hidden_forward, hidden_backward, Predicate_idx_batch, unlabeled_sentence, SRLprobs_teacher, unlabeled_lengths):
        ...
        tag_space = self.W_R_FB(hidden_states_word, hidden_states_predicate).view(
            self.batch_size, len(unlabeled_sentence[0]), -1)
        ...
        tag_space = self.W_R_BF(hidden_states_word, hidden_states_predicate).view(
            self.batch_size, len(unlabeled_sentence[0]), -1)
