...
class BiLSTMTagger(nn.Module):
    ...
    def __init__(self, hps, *_):
        ...
        self.W_R_PI = nn.Parameter(torch.rand(lstm_hidden_dim + 1, 2 * (lstm_hidden_dim + 1)))
    ...
    def CVT_train(self, sentence, p_sentence, sent_mask, lengths):
        ...
        for i in range(self.batch_size):
            random_index = np.random.randint(low=0, high=int(lengths[i]*0.6))
