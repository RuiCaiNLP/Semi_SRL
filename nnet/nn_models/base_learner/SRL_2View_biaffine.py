...
class BiLSTMTagger(nn.Module):
    ...
    def __init__(self, hps, *_):
        ...
        self.head_dropout_unlabeled_Past = nn.Dropout(p=0.1)
        self.dep_dropout_unlabeled_Past = nn.Dropout(p=0.1)
        self.head_dropout_unlabeled_Future = nn.Dropout(p=0.1)
        self.dep_dropout_unlabeled_Future = nn.Dropout(p=0.1)
        ...
        self.hidLayerFOH_SRL_Past = nn.Linear(self.ldims, self.biaffine_mid)
        self.hidLayerFOM_SRL_Past = nn.Linear(self.ldims, self.biaffine_mid)
        self.W_R_SRL_Past = nn.Parameter(torch.rand(self.biaffine_mid + 1, self.tagset_size * (self.biaffine_mid + 1)))

        self.hidLayerFOH_SRL_Future = nn.Linear(self.ldims, self.biaffine_mid)
        self.hidLayerFOM_SRL_Future = nn.Linear(self.ldims, self.biaffine_mid)
        self.W_R_SRL_Future = nn.Parameter(torch.rand(self.biaffine_mid + 1, self.tagset_size * (self.biaffine_mid + 1)))
    ...
    def init_SA_primary(self):
        return (
            torch.zeros(self.SA_primary_num_layers * 2, self.batch_size, self.hidden_dim, requires_grad=False).to(
                device),
            torch.zeros(self.SA_primary_num_layers * 2, self.batch_size, self.hidden_dim, requires_grad=False).to(
                device))
    ...
    def init_SRL_primary(self):
        return (
            torch.zeros(self.SRL_primary_num_layers * 2, self.batch_size, self.hidden_dim, requires_grad=False).to(
                device),
            torch.zeros(self.SRL_primary_num_layers * 2, self.batch_size, self.hidden_dim, requires_grad=False).to(
                device))
    ...
    def Semi_SRL_Loss(self, hidden_forward, hidden_backward, TagProbs_use, sentence, lengths, target_idx_in):
        ...
        predicate_embeds = hidden_backward[np.arange(0, self.batch_size), target_idx_in]
        Head_hidden = self.head_dropout_unlabeled_Past(F.relu(self.hidLayerFOH_SRL_Past(predicate_embeds)))
        Dependent_hidden = self.dep_dropout_unlabeled_Past(F.relu(self.hidLayerFOM_SRL_Past(hidden_past)))
        bias_one = torch.ones((self.batch_size, len(sentence[0]), 1)).to(device)
        Dependent_hidden = torch.cat((Dependent_hidden, Variable(bias_one)), 2)
        bias_one = torch.ones((self.batch_size, 1)).to(device)
        Head_hidden = torch.cat((Head_hidden, Variable(bias_one)), 1)
        left_part = torch.mm(Dependent_hidden.view(self.batch_size * len(sentence[0]), -1), self.W_R_SRL_Past)
        left_part = left_part.view(self.batch_size, len(sentence[0]) * self.tagset_size, -1)
        Head_hidden = Head_hidden.view(self.batch_size, -1, 1)
        tag_space = torch.bmm(left_part, Head_hidden).view(self.batch_size, len(sentence[0]), self.tagset_size)
        tag_space = tag_space.view(self.batch_size, len(sentence[0]), -1)
        ...
        predicate_embeds = hidden_forward[np.arange(0, self.batch_size), target_idx_in]
        Head_hidden = self.head_dropout_unlabeled_Future(F.relu(self.hidLayerFOH_SRL_Future(predicate_embeds)))
        Dependent_hidden = self.dep_dropout_unlabeled_Future(F.relu(self.hidLayerFOM_SRL_Future(hidden_future)))
        bias_one = torch.ones((self.batch_size, len(sentence[0]), 1)).to(device)
        Dependent_hidden = torch.cat((Dependent_hidden, Variable(bias_one)), 2)
        bias_one = torch.ones((self.batch_size, 1)).to(device)
        Head_hidden = torch.cat((Head_hidden, Variable(bias_one)), 1)
        left_part = torch.mm(Dependent_hidden.view(self.batch_size * len(sentence[0]), -1), self.W_R_SRL_Future)
        left_part = left_part.view(self.batch_size, len(sentence[0]) * self.tagset_size, -1)
        Head_hidden = Head_hidden.view(self.batch_size, -1, 1)
        tag_space = torch.bmm(left_part, Head_hidden).view(self.batch_size, len(sentence[0]), self.tagset_size)
        tag_space = tag_space.view(self.batch_size, len(sentence[0]), -1)
