...
class BiLSTMTagger(nn.Module):
    ...
    def __init__(self, hps, *_):
        ...
        self.hidLayerFOH_SRL_FF_2 = nn.Linear(self.ldims, self.biaffine_mid)
        self.hidLayerFOM_SRL_FF_2 = nn.Linear(self.ldims, self.biaffine_mid)
        self.W_R_SRL_FF_2 = nn.Parameter(torch.rand(self.biaffine_mid + 1, self.tagset_size * (self.biaffine_mid + 1)))

        self.hidLayerFOH_SRL_BB_2 = nn.Linear(self.ldims, self.biaffine_mid)
        self.hidLayerFOM_SRL_BB_2 = nn.Linear(self.ldims, self.biaffine_mid)
        self.W_R_SRL_BB_2 = nn.Parameter(torch.rand(self.biaffine_mid + 1, self.tagset_size * (self.biaffine_mid + 1)))

        self.hidLayerFOH_SRL_BF_2 = nn.Linear(self.ldims, self.biaffine_mid)
        self.hidLayerFOM_SRL_BF_2 = nn.Linear(self.ldims, self.biaffine_mid)
        self.W_R_SRL_BF_2 = nn.Parameter(torch.rand(self.biaffine_mid + 1, self.tagset_size * (self.biaffine_mid + 1)))

        self.hidLayerFOH_SRL_FB_2 = nn.Linear(self.ldims, self.biaffine_mid)
        self.hidLayerFOM_SRL_FB_2 = nn.Linear(self.ldims, self.biaffine_mid)
        self.W_R_SRL_FB_2 = nn.Parameter(torch.rand(self.biaffine_mid + 1, self.tagset_size * (self.biaffine_mid + 1)))
    ...
    def Semi_SRL_Loss(self, hidden_forward, hidden_backward, TagProbs_use, sentence, lengths, target_idx_in):
        ...
        ## Dependency Extractor FF_2
        predicate_embeds = hidden_forward[np.arange(0, self.batch_size), target_idx_in]
        Head_hidden = self.head_dropout_unlabeled_FF_2(F.relu(self.hidLayerFOH_SRL_FF_2(predicate_embeds)))
        Dependent_hidden = self.dep_dropout_unlabeled_FF_2(F.relu(self.hidLayerFOM_SRL_FF_2(hidden_forward)))
        bias_one = torch.ones((self.batch_size, len(sentence[0]), 1)).to(device)
        Dependent_hidden = torch.cat((Dependent_hidden, Variable(bias_one)), 2)
        bias_one = torch.ones((self.batch_size, 1)).to(device)
        Head_hidden = torch.cat((Head_hidden, Variable(bias_one)), 1)
        left_part = torch.mm(Dependent_hidden.view(self.batch_size * len(sentence[0]), -1), self.W_R_SRL_FF_2)
        left_part = left_part.view(self.batch_size, len(sentence[0]) * self.tagset_size, -1)
        Head_hidden = Head_hidden.view(self.batch_size, -1, 1)
        tag_space = torch.bmm(left_part, Head_hidden).view(self.batch_size, len(sentence[0]), self.tagset_size)
        tag_space = tag_space.view(self.batch_size, len(sentence[0]), -1)
        dep_tag_space = tag_space
        DEPprobs_student = F.log_softmax(dep_tag_space, dim=2)
        DEP_FF_loss_2 = unlabeled_loss_function(DEPprobs_student, TagProbs_use_softmax)

        ## Dependency Extractor BB
        predicate_embeds = hidden_backward[np.arange(0, self.batch_size), target_idx_in]
        Head_hidden = self.head_dropout_unlabeled_BB_2(F.relu(self.hidLayerFOH_SRL_BB_2(predicate_embeds)))
        Dependent_hidden = self.dep_dropout_unlabeled_BB_2(F.relu(self.hidLayerFOM_SRL_BB_2(hidden_backward)))
        bias_one = torch.ones((self.batch_size, len(sentence[0]), 1)).to(device)
        Dependent_hidden = torch.cat((Dependent_hidden, Variable(bias_one)), 2)
        bias_one = torch.ones((self.batch_size, 1)).to(device)
        Head_hidden = torch.cat((Head_hidden, Variable(bias_one)), 1)
        left_part = torch.mm(Dependent_hidden.view(self.batch_size * len(sentence[0]), -1), self.W_R_SRL_BB_2)
        left_part = left_part.view(self.batch_size, len(sentence[0]) * self.tagset_size, -1)
        Head_hidden = Head_hidden.view(self.batch_size, -1, 1)
        tag_space = torch.bmm(left_part, Head_hidden).view(self.batch_size, len(sentence[0]), self.tagset_size)
        tag_space = tag_space.view(self.batch_size, len(sentence[0]), -1)
        dep_tag_space = tag_space
        DEPprobs_student = F.log_softmax(dep_tag_space, dim=2)
        DEP_BB_loss_2 = unlabeled_loss_function(DEPprobs_student, TagProbs_use_softmax)

        ## Dependency Extractor FB
        predicate_embeds = hidden_forward[np.arange(0, self.batch_size), target_idx_in]
        Head_hidden = self.head_dropout_unlabeled_FB_2(F.relu(self.hidLayerFOH_SRL_FB_2(predicate_embeds)))
        Dependent_hidden = self.dep_dropout_unlabeled_FB_2(F.relu(self.hidLayerFOM_SRL_FB_2(hidden_backward)))
        bias_one = torch.ones((self.batch_size, len(sentence[0]), 1)).to(device)
        Dependent_hidden = torch.cat((Dependent_hidden, Variable(bias_one)), 2)
        bias_one = torch.ones((self.batch_size, 1)).to(device)
        Head_hidden = torch.cat((Head_hidden, Variable(bias_one)), 1)
        left_part = torch.mm(Dependent_hidden.view(self.batch_size * len(sentence[0]), -1), self.W_R_SRL_FB_2)
        left_part = left_part.view(self.batch_size, len(sentence[0]) * self.tagset_size, -1)
        Head_hidden = Head_hidden.view(self.batch_size, -1, 1)
        tag_space = torch.bmm(left_part, Head_hidden).view(self.batch_size, len(sentence[0]), self.tagset_size)
        tag_space = tag_space.view(self.batch_size, len(sentence[0]), -1)
        dep_tag_space = tag_space
        DEPprobs_student = F.log_softmax(dep_tag_space, dim=2)
        DEP_FB_loss_2 = unlabeled_loss_function(DEPprobs_student, TagProbs_use_softmax)

        ## Dependency Extractor BF
        predicate_embeds = hidden_backward[np.arange(0, self.batch_size), target_idx_in]
        Head_hidden = self.head_dropout_unlabeled_BF_2(F.relu(self.hidLayerFOH_SRL_BF_2(predicate_embeds)))
        Dependent_hidden = self.dep_dropout_unlabeled_BF_2(F.relu(self.hidLayerFOM_SRL_BF_2(hidden_forward)))
        bias_one = torch.ones((self.batch_size, len(sentence[0]), 1)).to(device)
        Dependent_hidden = torch.cat((Dependent_hidden, Variable(bias_one)), 2)
        bias_one = torch.ones((self.batch_size, 1)).to(device)
        Head_hidden = torch.cat((Head_hidden, Variable(bias_one)), 1)
        left_part = torch.mm(Dependent_hidden.view(self.batch_size * len(sentence[0]), -1), self.W_R_SRL_BF_2)
        left_part = left_part.view(self.batch_size, len(sentence[0]) * self.tagset_size, -1)
        Head_hidden = Head_hidden.view(self.batch_size, -1, 1)
        tag_space = torch.bmm(left_part, Head_hidden).view(self.batch_size, len(sentence[0]), self.tagset_size)
        tag_space = tag_space.view(self.batch_size, len(sentence[0]), -1)
        dep_tag_space = tag_space
        DEPprobs_student = F.log_softmax(dep_tag_space, dim=2)
        DEP_BF_loss_2 = unlabeled_loss_function(DEPprobs_student, TagProbs_use_softmax)
        ...
        DEP_FF_loss_2 = torch.sum(DEP_FF_loss_2, dim=2)
        DEP_BB_loss_2 = torch.sum(DEP_BB_loss_2, dim=2)
        DEP_BF_loss_2 = torch.sum(DEP_BF_loss_2, dim=2)
        DEP_FB_loss_2 = torch.sum(DEP_FB_loss_2, dim=2)
        ...
        DEP_Semi_loss = wordBeforePre_mask*(DEP_FF_loss+DEP_BB_loss+DEP_FB_loss+DEP_BF_loss)\
                      + wordAfterPre_mask*(DEP_FF_loss_2+DEP_BB_loss_2+DEP_FB_loss_2+DEP_BF_loss_2)
    ...
    def CVT_train(self, sentence, p_sentence, target_idx_in, lengths):
        ...
        Predicate_idx_batch = target_idx_in
    ...
    def forward(self, sentence, p_sentence, pos_tags, sent_mask, lengths, target_idx_in, region_marks,
        ...
        if cvt_train:
            CVT_SRL_Loss = self.CVT_train(sentence, sentence, target_idx_in,
                                          lengths)
            return CVT_SRL_Loss
