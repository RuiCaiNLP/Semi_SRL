...
class BiLSTMTagger(nn.Module):
    ...
    def forward(self, sentence, p_sentence,  pos_tags, lengths, target_idx_in, region_marks,
        ...
        # predicate identification
        Hidden_states_forID = torch.cat((hidden_states_0, hidden_states_1), 2)
        Predicate_identification = self.Idenficiation(self.Idenficiation_dropout(F.relu(self.MLP_identification(Hidden_states_forID))))
        Predicate_identification_space = Predicate_identification.view(
            len(sentence[0]) * self.batch_size, -1)


        # dependency extractor
        predicate_hidden_0 = hidden_states_0[np.arange(0, hidden_states_0.size()[0]), target_idx_in]
        predicate_hidden_1 = hidden_states_1[np.arange(0, hidden_states_1.size()[0]), target_idx_in]

        Word_hidden = self.DEP_proj_word_dropout(F.relu(self.hidden2tag_1(torch.cat((hidden_states_0, hidden_states_1), 2))))
        Predicate_hidden = self.DEP_proj_predicate_dropout(F.relu(self.hidden2tag_2(torch.cat((predicate_hidden_0, predicate_hidden_1), 1))))
