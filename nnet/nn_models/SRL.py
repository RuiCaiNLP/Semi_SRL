...
class BiLSTMTagger(nn.Module):
    ...
    def __init__(self, hps, *_):
        ...
        self.DEP_link_H_dropout = nn.Dropout(p=0.3)
        self.DEP_link_M_dropout = nn.Dropout(p=0.3)
        self.DEP_tag_H_dropout = nn.Dropout(p=0.3)
        self.DEP_tag_M_dropout = nn.Dropout(p=0.3)
    ...
    def forward(self, sentence, p_sentence, pos_tags, lengths, target_idx_in, region_marks,
        ...
        SRL_hidden_states = torch.cat((embeds_SRL,  fixed_embeds, pos_embeds, region_marks, SRL_composer), 2)
